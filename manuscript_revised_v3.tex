...
\begin{abstract}
Most bearing RUL studies rely on piecewise labels with an 80\% plateau and random time-step splitting. Using a reproducible fast protocol on XJTU-SY, we compare three models (Linear Regression, LSTM, DBCA-Net) under piecewise, linear, and exponential labels, with leave-one-bearing-out cross-validation and a constant predictor baseline. Under LOOCV, piecewise labels improve $R^2$ by about 0.20--0.23 over linear labels (for example, DBCA-Net from 0.238 to 0.442). Random time-step splitting adds only a small further gain in this protocol (piecewise random $R^2$ 0.433--0.459), so large near-perfect scores reported in the literature require careful verification before they are used as baselines. We propose a STAR reporting template: continuous labels, leave-one-bearing-out CV, constant baselines, and multi-model reporting. We also present an adaptive labeling strategy as a practical intermediate. The paper is intended as a methodological critique with constructive alternatives, not a new benchmark.
\end{abstract}
...
\begin{enumerate}
    ...
    \item Reproducible XJTU-SY comparison of three models and three labeling strategies under LOOCV, with a constant baseline and explicit reporting protocol. The results quantify label-induced differences and show that commonly reported near-perfect scores are not reproduced by the current protocol.
...
All deep models use AdamW, MSE loss, batch size 64, cosine annealing, 5 epochs for the fast reproducible protocol, and fixed seed 42. Results are reported as mean across folds; per-fold values and standard deviations are available in the reproducibility artifacts. A constant predictor baseline is reported separately.
...
\begin{itemize}
    \item \textbf{Piecewise vs.\ Linear:} Under LOOCV, DBCA-Net achieves $R^2 = 0.442$, LSTM $0.407$, and Linear Regression $0.408$ under piecewise labels. Under linear labels, $R^2$ falls to $0.238$, $0.206$, and $0.178$, respectively.
...
\begin{table}[t]
\centering
\caption{Performance comparison across RUL strategies (XJTU-SY, LOOCV, fast reproducible protocol, 5 epochs).}
\label{tab:results}
\small
...
LSTM & 0.184 & 0.407 & 0.256 & 0.206 & 0.237 & 0.235 \\
Linear Reg. & 0.183 & 0.408 & 0.262 & 0.178 & 0.237 & 0.233 \\
\midrule
Constant mean & 0.238 & 0.000 & 0.289 & 0.000 & 0.270 & 0.000 \\
\bottomrule
\end{tabular}}
\end{table}
...
\subsection{Label and Split Effects}
...
\begin{itemize}
    \item \textbf{Label choice under LOOCV:} piecewise labels increase $R^2$ by roughly 0.20--0.23 over linear labels across all three models.
    \item \textbf{Random splitting:} random time-step splitting adds only a small further gain in the fast protocol (piecewise random $R^2$ 0.433--0.459).
    \item \textbf{Interpretation:} the near-perfect random-split values reported in several papers are not reproduced by the current code, so no single inflation multiplier is claimed in this version.
\end{itemize}

\begin{table}[t]
\centering
\caption{$R^2$ across label types and evaluation splits (fast reproducible protocol, 5 epochs).}
\label{tab:inflation}
\small
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lccc}
...
\midrule
DBCA-Net & 0.238 & 0.442 & 0.459 \\
LSTM & 0.206 & 0.407 & 0.454 \\
Linear Reg. & 0.178 & 0.408 & 0.433 \\
\bottomrule
\end{tabular}}
\end{table}
...
A critical finding is that the ranking is consistent across model choice: all three models score higher under piecewise labels than under linear labels and show the same ordering piecewise $>$ exponential $>$ linear. Random splitting does not produce the near-perfect scores that appear in several published reports when the same fast protocol is used. This consistency is evidence that evaluation choices change apparent performance, but the exact magnitude depends on the training protocol and must be reported transparently.
...
SHAP analysis \cite{lundberg2017unified} identifies RMS and Variance as the most influential features (mean $|\text{SHAP}| > 0.02$), followed by Kurtosis and Crest Factor. Spectral Entropy shows the lowest importance. This ordering aligns with physical expectations: vibration amplitude and variability are primary degradation indicators.
...
We do not yet include a PHM2012 validation in this revision. The public raw data required for the same preprocessing pipeline is not part of the current artifact, and we do not want to report external-validation numbers that cannot be reproduced from the code and data provided. The inflation decomposition in this paper is therefore established on XJTU-SY; PHM2012, PU, and IMS validation are the first priority for extension.

Until those runs are available, we do not claim cross-dataset confirmation, and we do not use a single inflation multiplier. The reported pattern should be read as an XJTU-SY result with a clearly specified protocol. Two additional public datasets would strengthen the generalization argument: PU, with more than 200 run-to-failure experiments, and IMS, with four run-to-failure tests. Based on the entropy argument in Section~2.3, we expect inflation on PU and IMS to remain in a similar range, but that remains a prediction until the experiments are run.
...
\begin{enumerate}
    ...
    \item \textbf{Leave-one-bearing-out cross-validation.} Ensure no temporal overlap between training and test sets; report mean across folds, with per-fold values and standard deviations in the reproducibility artifacts.
    \item \textbf{Report constant predictor baseline.} A model always predicting the mean RUL defines the lower bound ($R^2 = 0$).
...
Across three models of increasing complexity (Linear Regression, LSTM, DBCA-Net), our reproducible XJTU-SY comparison shows that piecewise labels substantially change $R^2$ under leave-one-bearing-out evaluation, while random splitting adds only a small further gain in the fast protocol. The same ranking appears across model classes, which points to the evaluation protocol rather than any single architecture. We propose the STAR framework with continuous RUL labels, leave-one-bearing-out cross-validation, constant predictor baselines, and multi-model evaluation. External validation on PHM2012, PU, and IMS is the most important next step before any field-wide multiplier is claimed.
